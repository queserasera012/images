...
export const CONFIG = {
  ...
  family: {
    ...
    pupilDays: 6,
    studentDays: 7,
    // 老いる・旅立つ（D349）：生まれて55日で老人（仕事をやめて のんびり）。60〜90日で旅立つ（人ごとに ばらす）
    // D351：45日だと Day 50 に住民の64%が老人（引っ越してくる人は20〜30日で来るので、働ける日が短い）→ 55日（38%）
    elderAt: 55,
    lifeMin: 60,
    lifeMax: 90,
    ...
    // 島で生まれた子の名前（D357：12こでは Day 50 ごろ使い切り、「赤ちゃん」という名前の子が出ていた。使い切ったら 住民の名前から）
    kidNames: ['ハル', 'ユイ', 'レン', 'ミオ', 'ソウタ', 'ヒナ', 'リク', 'サクラ', 'カイ', 'ノア', 'アオイ', 'ツムギ',
      'ハルト', 'メイ', 'ユウマ', 'リン', 'アサヒ', 'ユナ', 'ミナト', 'エマ', 'シズク', 'ココナ', 'タクミ', 'ヒカリ',
      'ユウト', 'ワカナ', 'シュン', 'アカリ', 'ハヤト', 'ミツキ', 'コウ', 'ナナ', 'アユム', 'ユズ', 'ケント', 'モモ',
      'ヒロト', 'ミク', 'セナ', 'リオ', 'トワ', 'ユキナ', 'ルイ', 'ハナ', 'タイガ', 'サナ', 'ジン', 'ニコ'],
  },
  ...
  petshop: {
    ...
    max: 1,
    perArea: 1,                 // D327：島を1か所ひらくごとに1軒ふえる
    // ペットショップ&ブリーダー（D355）：カフェ&レストランと同じ「お店が一段上に変わる」。
    // 4日ごとに 子犬・子猫・子うさぎが1頭 入る（店に2頭まで）。ペットのいない家の人が 買いに来る。名前は あなたがつける
    breeder: {
      ...
      kinds: ['dog', 'cat', 'rabbit'], // キツネ・アライグマは野生なので売らない
      baby: { dog: '子犬', cat: '子猫', rabbit: '子うさぎ' },
      names: {
        dog: ['ハチ', 'コロ', 'チョコ', 'モモ', 'マロン', 'ソラ', 'ココ', 'ラッキー'],
        cat: ['タマ', 'クロ', 'トラ', 'ルナ', 'ミルク', 'ノエル', 'キナコ', 'シロ'],
        rabbit: ['ミミ', 'きなこ', 'ユキ', 'ラテ', 'おもち', 'ハナ', 'だいふく', 'ピョン'],
      },
    },
  },
